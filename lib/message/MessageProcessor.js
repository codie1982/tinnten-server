...
class SystemMessage extends BaseMessage {
  ...
  async processAndSave() {
    ...
    this.systemData = systemMessage.systemData || { recommendations: [] };

    return systemMessage
  }

  async saveQuestionsToDatabase() {
    // **Ürünler için soruları kaydet**
    if (this.request_type === "product" || this.request_type === "both") {
      for (const product of this.products) {
        if (product.question && product.question.length > 0) {
          for (const q of product.question) {
            let question = new Question();
            question.setQuestionText(q.q);
            question.setImportance(q.important);
            question.setInputType(q.input_type);
            question.setOptions(q.options);
            let savedQuestion = await new QuestionDB().create(question)
            this.productionQuestions.push(savedQuestion._id);
          }
        }
      }
    }

    // **Hizmetler için soruları kaydet**
    if (this.request_type === "service" || this.request_type === "both") {
      for (const service of this.services) {
        if (service.question && service.question.length > 0) {
          for (const q of service.question) {
            let question = new Question({
              questionText: q.q,
            ...
            let savedQuestion = await new QuestionDB().create(question)
            this.servicesQuestions.push(savedQuestion._id);
          }
        }
      }
    }
  ...
  async saveToDatabase() {
    ...
    message.setIntent(this.intent);
    message.setEntities(this.entities);
    message.setIncludeInContext(this.includeInContext);
    message.setSearchContext(this.search_context);
    message.setProductionQuestions(this.productionQuestions);
    message.setServicesQuestions(this.servicesQuestions);
    message.setSystemData(this.systemData);

    console.log("message", message)
    const messageDB = new MessageDB();
    return await messageDB.create(message)
  }
