...
const recomSystemPromt = require("../system_promt/recomSystemPromt");
const BaseAgent = require("./BaseAgent");


class RecomAgent extends BaseAgent {
  constructor(model = "gpt-3.5-turbo", temperature = 0.2) {
    super(model, temperature);
  }

  async getRecommendation(user, conversation, human_message) {
    try {
      console.log("[RecomAgent] Fetching intent system prompt...");
      const system_message = await recomSystemPromt(user,conversation, human_message);
