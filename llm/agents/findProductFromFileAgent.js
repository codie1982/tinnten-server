...
const findProductPrompt = require("../system_promt/findProductPrompt");
const BaseAgent = require("./BaseAgent");


class FindProduct extends BaseAgent {
  constructor(model = "gpt-3.5-turbo", temperature = 0.2) {
    super(model, temperature);
  }

  async create(companyid, human_message) {
    try {
      console.log("[FindProduct] Fetching intent system prompt...");
      const system_message = await findProductPrompt(companyid);
